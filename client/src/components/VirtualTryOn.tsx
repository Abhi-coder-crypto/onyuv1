...
export function VirtualTryOn({ garmentUrl, onSizeDetected }: TryOnProps) {
  ...
  useEffect(() => {
    ...
    loader.load(garmentUrl, (texture) => {
      ...
      // Create Torso Mesh with segments for deformation
      const torsoGeo = new THREE.PlaneGeometry(1, 1, 4, 4);
  ...
  useEffect(() => {
    ...
    poseInstance.onResults((results) => {
      ...
      const getSmooth = (idx: number) => {
        if (!smoothers.current[idx]) smoothers.current[idx] = new EMASmoother(0.1); // Increased smoothing
